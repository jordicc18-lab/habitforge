...
function Journal() {
  ...
  const addEntry = async () => {
    ...
    setText(''); setEmoji('😊'); setShowForm(false)
  }
    ...
    <PageTransition>
      <div className="pt-14 md:pt-0">
        ...
        <div className="relative mb-6">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar entradas..." className="w-full bg-gray-900 border border-gray-800 rounded-lg pl-9 pr-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-violet-500" />
        </div>
        ...
        {showForm && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6">
            ...
            <div className="flex gap-2 mb-3 flex-wrap">
              {EMOJIS.map(e => (
                <button key={e} onClick={() => setEmoji(e)} className={`text-2xl p-1 rounded-lg transition-all ${emoji === e ? 'bg-violet-600' : 'hover:bg-gray-800'}`}>{e}</button>
              ))}
            </div>
            <textarea value={text} onChange={e => setText(e.target.value)} placeholder="¿Cómo ha ido el día?" rows={4} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-violet-500 resize-none mb-3" />
            <div className="flex gap-2">
              <button onClick={addEntry} className="bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-lg transition-colors">Guardar</button>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 text-sm">Pulsa "Nueva" para empezar</p>
          </div>
        ) : (
