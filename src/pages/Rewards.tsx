...
function Rewards() {
  ...
  const toggleReward = async (reward: Reward) => {
    ...
    await updateDoc(ref, { unlocked: !reward.unlocked })
  }
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6">
          ...
          <div className="flex flex-col gap-3">
            <input
              ...
              placeholder="Ej: Comprar un libro nuevo"
            ...
            <input
              ...
              placeholder="Objetivo para conseguirla (ej: 30 días de racha)"
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-violet-500"
            />
          ...
          {rewards.map(reward => (
            <div
              ...
              className={`flex items-center justify-between bg-gray-900 border rounded-xl px-5 py-4 ${
                reward.unlocked ? 'border-yellow-500/50' : 'border-gray-800'
              }`}
            >
              <div className="flex items-center gap-4">
                <Gift size={20} className={reward.unlocked ? 'text-yellow-400' : 'text-gray-600'} />
                <div>
                  <p className={`font-medium ${reward.unlocked ? 'text-yellow-400' : 'text-white'}`}>
                    {reward.title}
                  </p>
                  {reward.goal && <p className="text-gray-500 text-sm">Objetivo: {reward.goal}</p>}
                  {reward.description && <p className="text-gray-500 text-sm">{reward.description}</p>}
                </div>
              </div>
