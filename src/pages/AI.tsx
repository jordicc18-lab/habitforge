...
function generateInsights(habits: Habit[]) {
  ...
  const bestHabit = habits.reduce((a, b) => getLastWeek(a) >= getLastWeek(b) ? a : b)
  const worstHabit = habits.reduce((a, b) => getLastWeek(a) <= getLastWeek(b) ? a : b)
  const totalCompletions = habits.reduce((acc, h) => acc + h.completedDates.length, 0)
  const bestStreakHabit = habits.reduce((a, b) => getBestStreak(a) >= getBestStreak(b) ? a : b)
  const insights = []

  if (pctToday === 100) insights.push({ type: 'success', text: '🎉 ¡Has completado el 100% de tus hábitos hoy! Día perfecto.' })
  else if (pctToday >= 50) insights.push({ type: 'info', text: `📊 Hoy llevas ${completedToday} de ${habits.length} hábitos completados (${pctToday}%). ¡Buen ritmo!` })
  else insights.push({ type: 'warning', text: `⚡ Hoy solo llevas ${completedToday} de ${habits.length} hábitos. Aún estás a tiempo de mejorar.` })

  insights.push({ type: 'success', text: `⭐ Tu hábito más constante esta semana es "${bestHabit.name}" con ${getLastWeek(bestHabit)}/7 días.` })
  ...
  insights.push({ type: 'info', text: `📈 En total llevas ${totalCompletions} completados entre todos tus hábitos.` })

  if (totalCompletions > 50) insights.push({ type: 'success', text: '💪 Llevas más de 50 completados en total. La constancia está dando sus frutos.' })
  else if (totalCompletions > 20) insights.push({ type: 'info', text: '🌱 Vas por buen camino. La clave es la consistencia día a día.' })
  else insights.push({ type: 'info', text: '🚀 Estás empezando. Los primeros 21 días son los más difíciles — no te rindas.' })
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
          ...
          {habits.length === 0 ? (
            ...
            <>
              ...
              <button onClick={() => setShowAnalysis(true)} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white px-4 py-2.5 rounded-lg transition-colors">
